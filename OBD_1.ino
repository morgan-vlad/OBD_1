...
#include "DEFINES.h"
#define Ls 0.00788 //производительсность форсунки литров в секунду // базовый 0.004 или 240cc
...
#define TOGGLE_BTN_PIN 3 //screen change PIN 4
#if defined(INJECTOR)
#define INJECTOR_PIN 4 //engine injector PIN 3
...
float cycle_obd_inj_dur, trip_obd_inj_dur, total_obd_inj_dur, trip_obd_fuel_consumption, total_obd_fuel_consumption, trip_obd_avg_fuel_consumption, total_obd_avg_fuel_consumption; //по обд протоколу
...
float getOBDdata(byte OBDdataIDX) {
  ...
  switch (OBDdataIDX) {
    ...
    case OBD_MAP: //Расходомер воздуха (MAP/MAF)
      ...
      returnValue = ToyotaData[OBD_MAP] * 0.97; //Сырые данные
      break;
